...
class QADoc2VecModel:
    ...
    def _get_question_vector(self, message):
        self.model_doc2vec.random.seed(self.model_doc2vec.seed)
        question_vector = self.model_doc2vec.infer_vector(self._tokenize_doc2vec(message)).reshape(1, -1)
        question_vector = normalize(question_vector).reshape(1, -1).astype('float32')
    ...
    def predict(self, message):
        ...
        question_vector = self._get_question_vector(message)
        ...
        self._get_similar_contexts(indices, distances, list_context_for_show, list_distance_for_show, k)
